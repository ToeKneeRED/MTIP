...
void Hooks::HookIAT(HMODULE hModule, const std::string& aFunctionToFind)
{
	...
	{
		"?", "@", "QEAA", "PEAV", "MEBAAV", "AEBV", "EEAAX", "MEAAX", "UEBAAV", "MEBAH", "PEBV", "HPEAPEAX", "UEBAPEBU",
		"MEBA", "UEAA1", "UEAA", "UEAAPEAXPEBD", "V$", "UEBA", "PEAX", "PEBD", "QEBA", "PEAX", "PEBD", "SAAV", "PEAA",
		"SAX", "PEAU", "AEAV", "YAX", "MEAA", "YAH", "_N", "MEAA", "CAH", "_J", "_K", "HPEA", "PEAH", "YAHPEBE00",
		"PEBU1", "$", "AV", "@H", "PEA01", "YA", "1", "PEBE00", "@X", "@U", "@Y", "SAA", "PEBH", "PEBU3", "00H", "AEAH",
		"W421", "HHH", "@V", "AEA", "PEADH0", "PEAD", "H0", "HHV"
	};
	// <function> <class> <return type (if none then void)> <args> <possibly argument class (might be prepended with W4)>
	// function:	starts with 0 = constructor; starts with 1 = destructor
	// args:		starts with W4/4 = thing after is argument's class
	// args:		constructors may have an arg that starts with Q/QE/QE0?
			...
			while (originalFirstThunk->u1.AddressOfData != NULL)
			{
				...
				for (const auto& hash : hashedString)
				{
					...
					while ((pos = functionString.find(hash, pos)) != std::string::npos) 
					{
						...
						else if(hash == "?" || hash == "QEAA" || hash == "PEAV" || hash == "MEBAAV" || 
							hash == "AEBV" || hash == "EEAAX" || hash == "MEAAX" || hash == "UEBAAV" ||
							hash == "MEBAH" || hash == "PEBV" || hash == "HPEAPEAX" || hash == "UEBAPEBU" || 
							hash == "MEBA" || hash == "UEAA1" || hash == "UEAA" || hash == "UEAAPEAXPEBD" || 
							hash == "V$" || hash == "UEBA" || hash == "PEAX" || hash == "PEBD" || hash == "QEBA" || 
							hash == "PEAX" || hash == "PEBD" || hash == "SAAV" || hash == "PEAA" || hash == "SAX" ||
							hash == "PEAU" || hash == "AEAV" || hash == "YAX" || hash == "MEAA" || hash == "YAH" ||
							hash == "_N" || hash == "MEAA" || hash == "CAH" || hash == "_J" || hash == "_K" ||
							hash == "HPEA" || hash == "PEAH" || hash == "YAHPEBE00" ||hash == "PEBU1" || 
							hash == "$" || hash == "AV" || hash == "@H" || hash == "PEA01" ||hash == "YA" || 
							hash == "PEBE00" || hash == "@X" || hash == "@U" || hash == "@Y" ||hash == "SAA" || 
							hash == "PEBH" || hash == "PEBU3" || hash == "00H" || hash == "AEAH" ||hash == "W421" || 
							hash == "HHH" || hash == "@V" || hash == "AEA" || hash == "PEADH0" || hash == "PEAD" ||
							hash == "H0" || hash == "HHV")
						{
				...
				if (std::string(functionName->Name) == aFunctionToFind)
				{
					...
					//TrueIsDebuggerPresent = (IsDebuggerPresent_t)GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "IsDebuggerPresent");
				}
				++originalFirstThunk;
				++firstThunk;
			}
